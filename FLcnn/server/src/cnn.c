...
/* Layer_feedFwd(self, cutLayer)
   Start feed forward till nth(cut layer) layer
*/
void Layer_feedFwd(Layer* self, const int cutLayer){

	int curLayer = 1;
    /* Start feed forwarding. */
    Layer* layer = self->lnext;
    while (layer != NULL && curLayer <= cutLayer) {
        switch (layer->ltype) {
        case LAYER_FULL:
		...
		/*if(curLayer == cutLayer){
			Layer_dump(layer, stderr);
		}*/
        layer = layer->lnext;
		//curLayer++;
    }
